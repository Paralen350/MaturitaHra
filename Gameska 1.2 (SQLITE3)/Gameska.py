...
def connect_db():
    try:
        # Vytvoříme spojení se souborem databáze (pokud neexistuje, vytvoří se)
...
class UserManager:
    ...
    def login(self, username, password):
        ...
        if conn:
            cursor = conn.cursor()
            cursor.execute("SELECT username, password, score FROM uzivateletestpygame WHERE username = ?", (username,))
    ...
    def register(self, username, password):
        ...
        if conn:
            ...
            try:
                ...
                cursor.execute("SELECT username FROM uzivateletestpygame WHERE username = ?", (username,))
                ...
                cursor.execute("INSERT INTO uzivateletestpygame (username, password, score) VALUES (?, ?, ?)", 
    ...
    def update_score(self, new_score):
        if self.current_user and new_score > self.score:
            ...
            if conn:
                cursor = conn.cursor()
                cursor.execute("UPDATE uzivateletestpygame SET score = ? WHERE username = ?", 
